...
perCaseBar = function(data, name='Stacked barplot', file='Test',catLabs = NA, legendTitle = NA, C=100){
  ...
  return(list(percent=data, concordN=concordN, discordN=discordN, casePlot=p))
}
...
perPathBar = function(ratings, name='Percent of HER2 status assigned by each pathologist', file='Test',catLabs = NA, legendTitle = NA){
  ...
  return(list(pathCount=pBPath, pathPlot=p))
}

concTab = function(ratings){
  # take N(sample)xR(raters) matrix and convert to N(samples)xQ(categories) matrix
  # ratings[is.na(ratings)] = '_'
  ...
  #   x=x[-naI]
  # }
  categories=na.omit(x)
  #Get number of raters/pathologists
  r=ncol(ratings)
  #Remove samples with all NAs
  naCount = rowSums(is.na(ratings))
  naI = which(naCount>=r)
  if(length(naI)!=0){
    data = ratings[-naI,]
  }
  ...
  for(i in 1:q){
    r_ik[,i] = rowSums(ratings == categories[i], na.rm=T)
  }
  return(r_ik)
}
...
fillTable = function(df, metTab, groupN, noICC=F, perAgree = 100, kappa_onlyCompletelyRatedCases=T){
  ...
  conT = concTab(df)
  #Remove cases that do not have ratings for all raters?
  if(kappa_onlyCompletelyRatedCases){
    completeRatedCases = conT[rowSums(conT)==ncol(df),]
    print("Using data from cases that were completely rated for Fleiss' kappa...")
    print(paste0(nrow(completeRatedCases), " out of ", nrow(conT), " cases included..."))
    kap = concordance(completeRatedCases, test='Normal')
  } else{
