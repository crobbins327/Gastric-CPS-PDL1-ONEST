...
opa = function(df, perAgree = 100, removeNA=TRUE){
  #Determine number of samples in df
  n = nrow(df)
  temp = rep(0,n)
  #number of pathologists
  m = ncol(df)
  #Number of pathologists needed to aggree for the sample to be considered concordant
  pathAgree = ceiling(m*perAgree/100)
  if (pathAgree==m){ 
    for(j in 1:n){
    ...
    for(j in 1:n){
      ...
      #NA counts as a unique value, and are not removed from calculation in original ONEST
      if(removeNA){
        temp[j] = sort(table(comp, useNA='no'), decreasing=T)[1]
      } else{
        temp[j] = sort(table(comp, useNA='ifany'), decreasing=T)[1]
      }
    }
    #Find the overall percent agreement as total number with at least pathAgree agreement over samples
    opaVal = sum(temp>=pathAgree, na.rm = TRUE)/n 
  }
...
plotline = function(path, indi=1, color='red', metric='OPA', perAgree=100){
  n = nrow(path)
  m = ncol(path)
  a = rep(0,m-1)
  ...
  if(metric=='OPA'){
    for (i in 1:M){
      I = i+1
      concord[i] = unlist(opa(path[,1:I], perAgree))[1]
    }
...
rand_plotline = function(t, path, indi=1, color='red', metric='OPA', perAgree=100){
  path = path[,sample(ncol(path))]
  C = plotline(path, indi, color, metric, perAgree)
  return(C)
}
...
ONEST_plot_fromData = function(concord, plot_data, name, file, ylab = 'Overall Percent Agreement', color='black', percent=TRUE){
  ...
  plot_data$path_number = 2:m
  q = ggplot(data = plot_data) + 
    geom_line(aes(x=path_number, y=mean, linetype="Mean"), color=color, size=1) + 
    geom_point(aes(x=path_number, y=mean), color=color, size=3, pch=21) + 
    geom_line(aes(x=path_number, y=quant_2.5, linetype="95% CI"), color=color, size=1) + 
    geom_line(aes(x=path_number, y=quant_97.5, linetype="95% CI"), color=color, size=1) + 
    scale_x_continuous(breaks= seq(2,m,by=2), limits=c(2,NA)) +
    xlab('Number of Raters in Group') + ylab(ylab) + ggtitle(name) +
...
ONEST_plot = function(data, plotI=TRUE, color='black', metric = 'OPA', perAgree = 100, skipModeling=FALSE){
  # Size of the data
  n = nrow(data)
  m = ncol(data)
  size_case = n
  ...
  if (plotI){
    C = plotline(data,1,color,metric,perAgree = perAgree)
  ...
  ##Multiprocessing, not for Windows...
  t = seq(1,100)
  Cs = mclapply(t, rand_plotline, path=data, indi=0, color=color, metric=metric, perAgree = perAgree)
  concord = do.call(cbind.data.frame, Cs)
  colnames(concord) = NULL
  # for (i in 1:100) {
  #   concord[,i] = plotline(data[,sample(m)],0,color,metric,perAgree)
  # }
  ...
  # D = data.frame(matrix(rep(0,simN*(m-1)),ncol=simN))
  # D[,1:100]= concord
  # for (i in 101:simN){
  #   D[,i]  = plotline(data[,sample(m)],0,color,metric, perAgree)
  # }
